...
import pandas as pd
import sys
from schedule_retry import schedule_retry

def schedule_restrictive(data, companies):
    """Scheduling programm"""
    # Create the model.
    ...
    for i in rows:
        if ls_m_c[i][8] < 100 and ls_m_c[i][10] > 100:
            ...
            model.Add((grid[(i,3)] + grid[(i, 4)] + grid[(i, 5)] + grid[(i, 6)]) < 100)
            model.Add((grid[(i,8)]) < 100)
